...
| **结果等待** | 点击后等待加载动画（默认 30s，可调），到点截图判定一次；成功即收尾，否则进入重试循环 |
...
<p align="center">
  <sub><b>03 场次表</b> — 场次（可指定星期）与场次日历　·　<b>04 识别</b> — 模型连接、判定词表与「测试识别」</sub>
</p>
...
| `selftest.bat` | 环境自检（33 项，无需 API Key，约 15 秒，打包前必跑） |
...
Key 保存后面板只显示尾号提示，输入框留空表示不修改，「清除」按钮可删除已存的 Key。
...
```
 ...
 置信度 ≥ 阈值(默认 0.70)?
   ...
   是 ──┴──► 换算屏幕坐标 ──► 模拟点击
                                 │
                                 ▼
                    点击后等待加载动画(默认 30s),到点截图判定一次
                                 │
              成功 ──► 记下"本周已领" ──► 截图留档 + 通知 ✅
                                 │
              未成功(含仍在加载) ──► 关闭客户端 → 等重试间隔 → 重开重试
                          (最多 5 次,仍失败则记录并等下一场)
```
...
「场次日历」按整月排布：未来的日子按各场次勾选的星期标出计划时间胶囊，过去的日子标当天的
领取结果（绿「已领」/ 红「未成功」/ 灰「无需领」），点 ‹ › 可翻月；
顶栏与概览页的「下一场」也会带出日期与星期，避免把下周五的场次误看成今天。

> **为什么「等待结果」是单独一步**：点击后服务端要走一段发放流程（实测约 40 秒），这期间界面停在
> 加载动画上，既没有结果弹窗、也没有任何可匹配的文案。所以点击后不立刻截图，而是先等加载动画
> （默认 30 秒，识别页「点击后等待加载」可调），到点再截图判定一次：出结果按结果收，仍在加载或
> 没有结论就按未成功进入重试循环——若实际已领到，重开后的页面会显示「已领取」，下一轮自然按已领收尾。
...
| **面板来源校验** | 面板能退出程序、清空留档、改配置 —— Host 非 127.0.0.1 / localhost、或来源（Origin/Referer）是外部站点的请求一律 403，恶意网页既触发不了操作，也借 DNS rebinding 读不到配置 |
...
| **结果固定等待** | 点击后等加载动画（默认 30s）再判定一次，策略简单可预期；等待时长在识别页可调 |
| **卡片缺失复查** | 福利卡片不在画面上时原地等 5s 复查一次（排除晚渲染）；仍不在按未成功走统一的重试循环 |
...
├─ selftest.py            自检 33 项(无需 API Key)
...
点击后程序会等待加载动画（默认 30 秒，识别页「点击后等待加载」可调）再截图判定一次，
这期间界面停在加载动画上属正常现象。

如果**判定时仍在加载、每轮都报未成功**：确认客户端窗口在执行期间没被其它窗口遮住；到日志页回放
`attempt-N-after.png` 校验图，看界面当时究竟停在哪个状态。若服务端经常超过 30 秒才发放，
可把等待调大（如 60 秒），减少不必要的重试；反之调小可以让失败更早进入下一轮。
</details>
