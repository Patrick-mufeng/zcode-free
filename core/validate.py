...
PENDING = "pending"    # 已提交、界面仍在加载:没有结果,如何处置由执行器策略决定
NO_CARD = "no_card"    # 福利卡片不在画面上:复查一次排除晚渲染,仍不在则按重试处理
...
def judge_locate(res: dict, cfg) -> str:
    ...
    # 卡片明确不在画面上:可能当期确实没有可领的福利,也可能只是比主界面晚渲染。
    # 只在模型明确回答 False 时采信(字段缺失/为 None 都不算),避免把"没看到"当
    # "不存在"。是否重开重试由执行器策略决定:先原地复查一次,仍不在则进重开循环。
...
def judge_verify(res: dict, cfg, clicked: bool = False) -> str:
    ...
    注意 PENDING:点击后界面停在加载动画上、关键字也认不出——它表示"还没出结果"
    而不是"失败"。是否继续等由执行器策略决定(当前策略:固定等待后截一次图,
    仍在加载即按未成功进入重试循环,重开后页面状态会反映服务端的真实结论)。
